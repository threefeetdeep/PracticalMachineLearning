...
```{r echo=FALSE, missing_values, warning=FALSE}
...
### Ignoring Timestamps and 'window' Features
...
training %>% select(-all_of(NA_cols)) -> training_no_NA
...
system.time(rfmodel_1 <- train(classe ~ ., 
                 data=training_no_NA[,-c(3:7)],
                 trControl=train_control, 
                 method="rf"))
```
Our training took about four minutes.
...
```
Amazingly, the Random Forest classifier has performed with 100% accuracy on the 5,885 observations in the test set! This gives us confidence that the predictor will perform well on any new unseen data.
...
We see that PCA has roughly halved the number of features required, whilst still capturing 95% of the variance in the data. The hope was that this will help reduce the training time of our chosen classifier, but surprisingly, we see it actually increased from 244 to 262 seconds!
...
Perhaps not unsurprisingly, the classifier makes a few errors on the test set, reducing accuracy from 100% to 98.81%, as PCA does sacrifice some "detail" or variation in the data as the cost of reducing the number of dimensions. 
...
c$overall
```
We see this trains *very* quickly, in a couple of seconds, but has a **terrible** accuracy of about 66%. We will not use a basic model such as this for our final model.
...
rfpred
```
Our classifier thinks all 20 exercises in the final prediction test set are of Class A, i.e. exercise performed correctly.
